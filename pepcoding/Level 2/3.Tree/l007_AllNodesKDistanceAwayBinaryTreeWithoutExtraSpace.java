...
public class l007_AllNodesKDistanceAwayBinaryTreeWithoutExtraSpace {
    ...
    public static ArrayList<Integer> distanceK(TreeNode root, int target, int k) {
        ...
        distanceKHelper(root, target, k, list);
        return list;
    }

    public static int distanceKHelper(TreeNode node, int target, int k, ArrayList<Integer> list) {
        if (node == null)
            return -1;
        ...
        int left = distanceKHelper(node.left, target, k, list);
        if (left != -1) {
            kDown(node, node.left, k - left, list);
            return left + 1;
        }

        int right = distanceKHelper(node.right, target, k, list);
        if (right != -1) {
            kDown(node, node.right, k - right, list);
            return right + 1;
        }
