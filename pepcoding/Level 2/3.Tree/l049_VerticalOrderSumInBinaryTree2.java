import java.util.ArrayList;

// https://practice.geeksforgeeks.org/problems/vertical-sum/1

// Done using DFS - mySolution

public class l049_VerticalOrderSumInBinaryTree2 {
    class Node {
        int data;
        Node left, right;

        Node(int item) {
            data = item;
            left = right = null;
        }
    }

    public ArrayList<Integer> verticalSum(Node root) {
        ArrayList<Integer> ans = new ArrayList<Integer>();
        if (root == null)
            return ans;

        // getinng width of tree
        int[] minMax = new int[2];
        widthOfShadow(root, 0, minMax);
        int width = minMax[1] - minMax[0] + 1;

        // filling arraylist
        for (int idx = 0; idx < width; idx++)
            ans.add(0);

        verticalSum(root, Math.abs(minMax[0]), ans);

        return ans;
    }

    private void verticalSum(Node root, int vl, ArrayList<Integer> ans) {
        if (root == null)
            return;

        ans.set(vl, ans.get(vl) + root.data);

        verticalSum(root.left, vl - 1, ans);
        verticalSum(root.right, vl + 1, ans);
    }

    static class Pair {
        Node node = null;
        int vl = 0;

        Pair(Node node, int vl) {
            this.node = node;
            this.vl = vl;
        }
    }

    private static void widthOfShadow(Node root, int vl, int[] minMax) {
        if (root == null)
            return;

        minMax[0] = Math.min(minMax[0], vl);
        minMax[1] = Math.max(minMax[1], vl);

        widthOfShadow(root.left, vl - 1, minMax);
        widthOfShadow(root.right, vl + 1, minMax);
    }
}
